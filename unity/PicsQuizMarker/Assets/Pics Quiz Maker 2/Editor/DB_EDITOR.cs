...
public class ObjectBuilderEditor : Editor
{
	...
	string[] folder = {"Assets/Pics Quiz Maker 2/Images"};
	string[] allPathImages;
	bool showAutomaticSystemOfAdd = false;
	//variable for add all images automatically (userd in quit the number and _)
	...
	public override void OnInspectorGUI ()
	{
		...
		if (EditorPrefs.GetBool ("showConfigs")) {
            ...
            EditorGUILayout.LabelField("Restart the coins when the player finish all levels?");
            wordDb.restartCoinsWhenRestartTheGame = EditorGUILayout.Toggle("Yes/not", wordDb.restartCoinsWhenRestartTheGame);
		...
		if (EditorPrefs.GetBool ("showWords")) {
			...
			if (showAutomaticSystemOfAdd) {
				...
				EditorGUILayout.LabelField ("1. All images should be into the folder \"Images\"");
				...
				EditorGUILayout.LabelField ("3. Clic on the button to add automatically. (All levels added will be replaced)");
				...
				if (GUILayout.Button ("FIND ERRORS", GUILayout.Width (300))) {
					...
					Debug.Log("REVISION OF ERRORS COMPLETED");
				}
