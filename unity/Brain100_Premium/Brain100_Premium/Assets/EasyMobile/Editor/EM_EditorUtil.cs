...
namespace EasyMobile.Editor
{
    public static class EM_EditorUtil
    {
        ...
        /// <summary>
        /// Finds the game object with the specified name in the scenes at the given paths.
        /// Note that this method only looks for root level objects.
        /// </summary>
        /// <returns>The first game object found in scenes, or null if none was found.</returns>
        /// <param name="objectName">Object name.</param>
        /// <param name="scenePaths">Scene paths.</param>
        public static GameObject FindGameObjectInScenes(string objectName, string[] scenePaths)
        {
            ...
            for (int i = 0; i < scenePaths.Length; i++)
            {
                ...
                {
                    go = FindGameObjectInScene(objectName, activeScene);
                }
                else
                {
                    Scene scene = EditorSceneManager.OpenScene(scenePaths[i], OpenSceneMode.Additive);
                    go = FindGameObjectInScene(objectName, scene);
        ...
        /// <summary>
        /// Finds the instance of the specified prefab in scenes. Note that this method
        /// only looks for root level instances.
        /// </summary>
        /// <returns>The first prefab instance found in scenes, or null if none was found.</returns>
        /// <param name="prefab">Prefab.</param>
        /// <param name="scenePaths">Scene paths.</param>
        public static GameObject FindPrefabInstanceInScenes(GameObject prefab, string[] scenePaths)
        {
            var prefabType = PrefabUtility.GetPrefabType(prefab);
            if (prefabType != PrefabType.Prefab && prefabType != PrefabType.ModelPrefab)
            {
            ...
            for (int i = 0; i < scenePaths.Length; i++)
            {
                if (activeScene.path.Equals(scenePaths[i]))
                {
                    go = FindPrefabInstanceInScene(prefab, activeScene);
                }
                else
                {
                    Scene scene = EditorSceneManager.OpenScene(scenePaths[i], OpenSceneMode.Additive);
                    go = FindPrefabInstanceInScene(prefab, scene);
                    EditorSceneManager.CloseScene(scene, true);
                }
